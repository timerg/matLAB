...
for c = 1:Nbmp
  c
    ...
    for aa = 1: 1
        ...
        if rem(c,(Nbmp/10)) == 0,
            figure(5);title('wij')
            subplot(2, 10, c/Nbmp * 10); imshow(reshape((vi_origin .* 255), 28, 28), [0 255]);
            subplot(2, 10, 10 + c / Nbmp * 10); imshow(reshape((vi_1 .* 255), 28, 28), [0 255]);
        end
    end
end
%%% layer jk
  ...
  c
    ...
    for aa = 1: 1
        ...
        if rem(c,(Nbmp/10)) == 0,
            ...
            subplot(2, 10, 10 + c / Nbmp * 10); imshow(reshape((vi_2 .* 255), 28, 28), [0 255]);
            % subplot(2, 10, 30 + c / Nbmp * 10); imshow(reshape((vi_3 .* 255), 28, 28), [0 255]);
...
vi_3 = gt(pjir, rand(1, nin))';
        ...
        if rem(c,(Nbmp/10)) == 0,
            figure(6);title('wkt')
            subplot(2, 10, c/Nbmp * 10); imshow(reshape((vi_origin .* 255), 28, 28), [0 255]);
            subplot(2, 10, 10 + c / Nbmp * 10); imshow(reshape((vi_3 .* 255), 28, 28), [0 255]);
        end
    end
end
%%% distinguish
